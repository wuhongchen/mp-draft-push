...
- `R2_BUCKET` / `R2_PUBLIC_URL`：用于把生成的图片上传到 Cloudflare R2 并得到公网 URL。
...
Cloudflare R2 准备工作（最小化）：

- 安装 Wrangler：`npm i -g wrangler`
- 登录：`wrangler login`
- 创建 bucket：`wrangler r2 bucket create <your-bucket>`
- 给 bucket 配置可公开访问的域名，然后把公开基址填到 `R2_PUBLIC_URL`
